...
export function resolveClaudeWebV2Config(): ClaudeWebV2RuntimeConfig {
    ...
    const port = Number(process.env.CLAUDE_WEB_DEBUG_PORT ?? fileConfig.debugPort) || DEFAULT_DEBUG_PORT;
    return {
        ...
        debugPort: port,
        headless: process.env.CLAUDE_WEB_HEADLESS !== undefined
            ? process.env.CLAUDE_WEB_HEADLESS !== "false"
            : (fileConfig.headless ?? false),
        debug: process.env.CLAUDE_WEB_DEBUG !== undefined
            ? process.env.CLAUDE_WEB_DEBUG !== "false"
            : (fileConfig.debug ?? false),
        launchTimeoutMs: Number(process.env.CLAUDE_WEB_LAUNCH_TIMEOUT_MS ?? fileConfig.launchTimeoutMs) || DEFAULT_LAUNCH_TIMEOUT_MS,
        responseTimeoutMs: Number(process.env.CLAUDE_WEB_RESPONSE_TIMEOUT_MS ?? fileConfig.responseTimeoutMs) || DEFAULT_RESPONSE_TIMEOUT_MS,
        loginTimeoutMs: Number(process.env.CLAUDE_WEB_LOGIN_TIMEOUT_MS ?? fileConfig.loginTimeoutMs) || DEFAULT_LOGIN_TIMEOUT_MS,
        chatsFile: process.env.CLAUDE_WEB_CHATS_FILE || fileConfig.chatsFile || DEFAULT_CHATS_FILE,
        minSendDelayMs: Number(process.env.CLAUDE_WEB_MIN_SEND_DELAY_MS ?? fileConfig.minSendDelayMs) || DEFAULT_MIN_SEND_DELAY_MS,
        maxSendDelayMs: Number(process.env.CLAUDE_WEB_MAX_SEND_DELAY_MS ?? fileConfig.maxSendDelayMs) || DEFAULT_MAX_SEND_DELAY_MS,
        toolTurnExtraMinMs: Number(process.env.CLAUDE_WEB_TOOL_TURN_EXTRA_MIN_MS ?? fileConfig.toolTurnExtraMinMs) || DEFAULT_TOOL_TURN_EXTRA_MIN_MS,
        toolTurnExtraMaxMs: Number(process.env.CLAUDE_WEB_TOOL_TURN_EXTRA_MAX_MS ?? fileConfig.toolTurnExtraMaxMs) || DEFAULT_TOOL_TURN_EXTRA_MAX_MS,
    };
}
...
async function connectBrowser(config: ClaudeWebV2RuntimeConfig): Promise<CdpClient> {
    ...
    if (!executablePath) {
        throw new Error("Could not find Chrome. Set chromePath in ~/.cline/claude-web/config.json or CLAUDE_WEB_CHROME_PATH.");
    }
...
// ---------- 1. Toggle "Think" mode ----------
async function setClaudeThink(enable) {
...
async function clickClaudeSend() {
    ...
    if (!sendBtn || sendBtn.disabled) {
        sendBtn = Array.from(document.querySelectorAll('button')).find(btn => {
            ...
            return svg && label.includes('send');
        });
    }
...
async function sendMessageToClaude(message, options) {
    ...
    // Send the message
    await clickClaudeSend();
    console.log('✅ Message sent: ' + message);
    return true;
}
`;

function buildSendScript(prompt: string, options?: { think?: boolean }): string {
    const opts = options || {};
    return `(async () => {
        ${SEND_MESSAGE_SOURCE}
...
export function lookupClaudeChatSession(chatsFile: string, chatKey: string): string | undefined {
    return readChatRegistry(chatsFile)[chatKey]?.session_id;
}

export function recordClaudeChatSession(chatsFile: string, chatKey: string, sessionId: string): void {
...
export function deleteClaudeChatSession(chatsFile: string, chatKey: string): void {
    const registry = readChatRegistry(chatsFile);
    if (registry[chatKey]) {
...
export async function openClaudeWebChat(
    ...
    if (!pageTarget) {
        const result = await cdp.send("Target.createTarget", { url: CLAUDE_WEB_URL });
        await sleep(2000);
    ...
    return {
        sessionId,
        url: `https://claude.ai/c/${sessionId}`,
    };
}
...
function consumeClaudeSse(
    ...
    try {
        // Claude SSE parsing: see consumeClaudeSse doc below.
        let fullText = "";
...
async function readPageUrl(cdp: CdpClient, cdpSessionId: string): Promise<string> {
    try {
...
async function navigateClaudeChat(
    ...
    const destination = target.fresh
        ? CLAUDE_WEB_URL
        : target.sessionId
            ? `https://claude.ai/c/${target.sessionId}`
            : CLAUDE_WEB_URL;
    ...
    if (alreadyThere && !forceReload) {
        logger?.debug?.(`[claude-web] already on ${destination} — skipping navigation (no reload)`);
        if (target.fresh) {
            await cdp.send(
                "Runtime.evaluate",
...
async function sendAndCapture(
    ...
    const onLoadingFinished = async (event: any, eventSessionId?: string) => {
        ...
        try {
            ...
            capturedBody = base64Encoded ? Buffer.from(body, "base64").toString("utf-8") : body;
    ...
    try {
        ...
        debugLog(`pacing: waiting ${sendDelay}ms before send (toolTurn=${String(isToolTurn)})`);
        ...
        consumeClaudeSse(
            ...
            () => {},
            (err) => { logger?.error?.(`[claude-web] SSE parse error: ${err.message}`); },
...
function buildClaudePrompt(
    ...
    if (reInjectSystem && systemMessage && !alreadyHasSystem && conversation.length > 0) {
        return messagesToPrompt([systemMessage, ...conversation], promptOptions);
    }
    return messagesToPrompt(conversation, promptOptions);
}

function createClaudeWebModel(modelId: string, logger?: BasicLogger): LanguageModelV2 {
    ...
    async function runCompletion(
        ...
        const existingClaudeSession = lookupClaudeChatSession(runtimeConfig.chatsFile, chatKey);
        ...
        await navigateClaudeChat(
            cdp,
            cdpSessionId,
            existingClaudeSession ? { sessionId: existingClaudeSession, fresh: false } : { fresh: true },
            logger,
            forceReload,
        );
        ...
        const { cleanedContent, toolCalls } = parseDeepSeekToolCalls(result.text, toolNames);
        const looseCalls =
            toolCalls.length === 0 ? parseLooseDeepSeekToolCalls(result.text, toolNames) : toolCalls;
        if (looseCalls.length > 0) {
            const { tools: validatedCalls, retryPrompt } = validateToolCalls(looseCalls);
            const displayText = retryPrompt ? `${cleanedContent}\n\n${retryPrompt}`.trim() : cleanedContent;
            return { text: displayText, toolCalls: validatedCalls, usage: result.usage };
        }
        ...
        const fallback = parseFallbackToolUses(cleanedContent, lastUserText(options.prompt), toolNames);
        return { text: fallback.cleanedText, toolCalls: fallback.toolUses, usage: result.usage };
    }
    ...
    const provider: LanguageModelV2 = {
        ...
        async doStream(options: LanguageModelV2CallOptions) {
            ...
            parts.push({ type: "finish", finishReason: finishReasonFor(text, toolCalls), usage });
