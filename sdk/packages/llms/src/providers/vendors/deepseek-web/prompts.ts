 ...
 * `default` and `worker`.
 */
export const deepseekWebPrompts: WebProviderPrompts = {
	// The human-in-the-loop prompt: PowerShell to read with, a patch block to
	// edit with. Unlike the other web providers this one talks to chat.deepseek.com
	// over HTTP rather than a scraped browser tab, but the model on the far end is
	// still the consumer chat product, not a function-calling API: handing it the
	// JSON tool contract makes it worse — it spends the turn formatting JSON
	// instead of thinking.
	//
	// `deepseek-web` is on the `web-chat-providers-use-apply-patch` rule in
