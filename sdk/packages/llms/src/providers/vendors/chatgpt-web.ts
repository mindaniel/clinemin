...
export function resolveChatGPTWebV2Config(): ChatGPTWebV2RuntimeConfig {
    ...
    const port = Number(process.env.CHATGPT_WEB_DEBUG_PORT ?? fileConfig.debugPort) || DEFAULT_DEBUG_PORT;
    return {
        ...
        debugPort: port,
        headless: process.env.CHATGPT_WEB_HEADLESS !== undefined
            ? process.env.CHATGPT_WEB_HEADLESS !== "false"
            : (fileConfig.headless ?? false),
        debug: process.env.CHATGPT_WEB_DEBUG !== undefined
            ? process.env.CHATGPT_WEB_DEBUG !== "false"
            : (fileConfig.debug ?? false),
        launchTimeoutMs: Number(process.env.CHATGPT_WEB_LAUNCH_TIMEOUT_MS ?? fileConfig.launchTimeoutMs) || DEFAULT_LAUNCH_TIMEOUT_MS,
        responseTimeoutMs: Number(process.env.CHATGPT_WEB_RESPONSE_TIMEOUT_MS ?? fileConfig.responseTimeoutMs) || DEFAULT_RESPONSE_TIMEOUT_MS,
        loginTimeoutMs: Number(process.env.CHATGPT_WEB_LOGIN_TIMEOUT_MS ?? fileConfig.loginTimeoutMs) || DEFAULT_LOGIN_TIMEOUT_MS,
        chatsFile: process.env.CHATGPT_WEB_CHATS_FILE || fileConfig.chatsFile || DEFAULT_CHATS_FILE,
        minSendDelayMs: Number(process.env.CHATGPT_WEB_MIN_SEND_DELAY_MS ?? fileConfig.minSendDelayMs) || DEFAULT_MIN_SEND_DELAY_MS,
        maxSendDelayMs: Number(process.env.CHATGPT_WEB_MAX_SEND_DELAY_MS ?? fileConfig.maxSendDelayMs) || DEFAULT_MAX_SEND_DELAY_MS,
        toolTurnExtraMinMs: Number(process.env.CHATGPT_WEB_TOOL_TURN_EXTRA_MIN_MS ?? fileConfig.toolTurnExtraMinMs) || DEFAULT_TOOL_TURN_EXTRA_MIN_MS,
        toolTurnExtraMaxMs: Number(process.env.CHATGPT_WEB_TOOL_TURN_EXTRA_MAX_MS ?? fileConfig.toolTurnExtraMaxMs) || DEFAULT_TOOL_TURN_EXTRA_MAX_MS,
    };
}
...
async function connectBrowser(config: ChatGPTWebV2RuntimeConfig): Promise<CdpClient> {
    ...
    if (!executablePath) {
        throw new Error("Could not find Chrome. Set chromePath in ~/.cline/chatgpt-web/config.json or CHATGPT_WEB_CHROME_PATH.");
    }
...
export function lookupChatGPTChatSession(chatsFile: string, chatKey: string): string | undefined {
    return readChatRegistry(chatsFile)[chatKey]?.session_id;
}

export function recordChatGPTChatSession(chatsFile: string, chatKey: string, sessionId: string): void {
...
export function deleteChatGPTChatSession(chatsFile: string, chatKey: string): void {
    const registry = readChatRegistry(chatsFile);
    if (registry[chatKey]) {
...
export async function openChatGPTWebChat(
    ...
    if (!pageTarget) {
        const result = await cdp.send("Target.createTarget", { url: CHATGPT_WEB_URL });
        await sleep(2000);
...
async function navigateChatGPTChat(
    ...
    if (alreadyThere && !forceReload) {
        logger?.debug?.(`[chatgpt-web] already on ${destination} — skipping navigation (no reload)`);
        if (target.fresh) {
            await cdp.send(
                "Runtime.evaluate",
...
async function sendAndCapture(
    ...
    try {
        ...
        consumeChatGPTSse(
            ...
            () => {},
            (err) => { logger?.error?.(`[chatgpt-web] SSE parse error: ${err.message}`); },
...
function createChatGPTWebModel(modelId: string, logger?: BasicLogger): LanguageModelV2 {
    ...
    async function runCompletion(
        ...
        const existingChatGPTSession = lookupChatGPTChatSession(runtimeConfig.chatsFile, chatKey);
        ...
        await navigateChatGPTChat(
            cdp,
            cdpSessionId,
            existingChatGPTSession ? { sessionId: existingChatGPTSession, fresh: false } : { fresh: true },
            logger,
            forceReload,
        );
        ...
        if (chatgptSession) {
            recordChatGPTChatSession(runtimeConfig.chatsFile, chatKey, chatgptSession);
        }
